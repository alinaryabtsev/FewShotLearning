...
SAVE_NAME = "support_analysis_k13_ku"
...
def get_lesions_areas(support_labels_patches):
    ...
    ## kutiu: sorted the list by area to facilitate future looping - @@ is this written correctly? was there a past assumption on being sortd by index?
    lesions_sorted = sorted(zip(leasions_areas, lesios_indices))

    lesion_areas = [a for a, _ in lesions_sorted]
    lesion_indices = [i for _, i in lesions_sorted]

    return lesion_areas, lesion_indices


def sample_lesions_from_gaussian_distribution(lesion_areas, lesion_indices):
    """
    ...
    :param lesion_areas: a list of the areas of the lesions
    :param lesion_indices: a list of the indices of the lesions
    ...
    # nz_bin_p = pdf_values[nz_idx]
    # nz_bin_p = nz_bin_p / np.sum(nz_bin_p)
    # nz_bin_p = nz_bin_p  / np.sum(nz_bin_p )
    ...
    np.random.seed(42)
    drawn_indices = np.random.choice(len(data_samples), size=NUM_OF_SAMPLED_PATCHES, replace=False,
                                     p=lesions_p)  # kutiu: draw lesions from the sorted list
    return np.array(lesion_indices)[drawn_indices]  # kutiu: respective lesion indices in the original dataset
