...
def load_data():
    d_support = LungTumorsDataset3D(split="support", support_frac=0.1, label=1, resize_scan=False)
    d_query = LungTumorsDataset3D(split="query", support_frac=0.5, label=1, resize_scan=False)
    return d_support, d_query


def get_support_images(d_support, K=5):
    K = K  # number of shots in support
...
def main():
    logging.basicConfig(filename='lungs_prediction.log', encoding='utf-8', level=logging.DEBUG,
                        format='%(asctime)s %(message)s', datefmt='%d/%m/%Y %I:%M:%S %p')
    logging.info("Started running lungs prediction")
